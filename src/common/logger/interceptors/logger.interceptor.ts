...
import { LOGGER_ACTION_META_KEY, LOGGER_OPTIONS_META_KEY } from 'src/common/logger/constants/logger.constant';
...
export class LoggerInterceptor implements NestInterceptor<any> {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<Promise<any> | string>> {
    if (context.getType() === 'http') {
      ...
      const { method, originalUrl, __id, body, params, path } = ctx.getRequest<IRequestApp>();
