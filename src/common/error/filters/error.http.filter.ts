...
export class ErrorHttpFilter implements ExceptionFilter {
  ...
  async catch(exception: unknown, host: ArgumentsHost): Promise<void> {
    ...
    const __requestId = request.__id ?? uuidV4();
        ...
        if (responseException.errors?.length > 0) {
          errors =
            responseException._errorType === ERROR_TYPE.IMPORT
              ? this.messageService.getImportErrorsMessage(responseException.errors as IValidationErrorImport[], {
                  customLanguages: __customLang,
                })
              : this.messageService.getRequestErrorsMessage(responseException.errors as ValidationError[], {
                  customLanguages: __customLang,
                });
        }
