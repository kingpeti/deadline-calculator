...
import { DEBUGGER_HTTP_FORMAT, DEBUGGER_HTTP_NAME } from 'src/common/debugger/constants/debugger.constant';
...
export class DebuggerHttpWriteIntoFileMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction): Promise<void> {
    if (this.writeIntoFile) {
      ...
      morgan(config.debuggerHttpFormat, config.debuggerHttpOptions)(req, res, next);
    } else {
      next();
    }
  }
